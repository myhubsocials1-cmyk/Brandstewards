...
void main() {
  ...
  float ripple = sin(dist * 20.0 - u_time * 5.0) * 0.005 * smoothstep(0.5, 0.0, dist);
  vec2 offset = vec2(ripple);
  
  // Clean texture sample without chromatic aberration to keep image sharp
...
export default function VideoShaderBg({ 
  videoUrl, 
  className = "" 
}: { 
  videoUrl: string;
